<div id="imageContainer">
	<div id="largeImage" class="{if $images|@count == 0}missingImage{/if} {if $images|@count > 1}multipleImages{/if}">
		{if $product.DefaultImage.paths.3}
			<a onclick="Product.Lightbox2Gallery.start(this); return false;" href="{$product.DefaultImage.paths.4}" title="{$product.DefaultImage.title_lang|escape}" target="_blank">
				<img src="{$product.DefaultImage.paths.3}" alt="{$product.name_lang|escape} {$request.variation.sku|default:$product.sku}" id="mainImage" />
			</a>
		{else}
			{img src='MISSING_IMG_LARGE'|config alt="" id="mainImage"}
		{/if}
					{*include file="module/store-cloning/mirror/button.tpl"*}
		{if $product.hasMirror && 'ENABLE_VIRTUAL_MIRROR'|config}
		<a href="#" value="Try On" id="try-on" onclick="showMirror({$product.ID}, event)">Try Them On</a>
		{/if}
	</div>
	{if $images|@count > 1}
		<div id="moreImages">
	...
	{if $images|@count > 0}
		...
		<div class="hidden">
			{foreach from=$images item="image"}
				<a rel="lightbox[product]" href="{$image.paths.4}" target="_blank" onclick="return false;">{img src=$image.paths.1 id="img_`$image.ID`" alt=$image.title_lang|escape onclick="return false;"}</a>
			{/foreach}
		</div>
	{/if}
