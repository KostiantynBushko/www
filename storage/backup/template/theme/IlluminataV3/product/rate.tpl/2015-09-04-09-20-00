...
{if 'ENABLE_REVIEWS'|config}
	<div class="reviewForm">
		<p{if $ratingForm|@isRequired:'nickname'} class="required"{/if}>
			{err for="nickname"}
				<label class="wide" style="padding-bottom: 0px !important;">{t _nickname}:</label>
				{textfield class="text wide"}
			{/err}
		</p>
		<p{if $ratingForm|@isRequired:'title'} class="required"{/if}>
			{err for="title"}
				<label class="wide" style="padding-bottom: 0px !important;">{t _summary}:</label>
				{textfield class="text wide"}
			{/err}
		</p>
		<p{if $ratingForm|@isRequired:'text'} class="required"{/if}>
			{err for="text"}
				<label class="wide" style="padding-bottom: 0px !important;">{t _review_text}:</label>
				{textarea}
			{/err}
		</p>
	</div>
	<p>
		<input class="submit" type="submit" value="{tn _submit_review}" style="margin-top: 20px;" /> <span class="progressIndicator" style="display: none;"></span>
	</p>
{/if}
