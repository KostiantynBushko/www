...
class Category extends ActiveTreeNode implements MultilingualObjectInterface, iEavFieldManager, EavAble
{
	...
	public static function recalculateProductsCount()
	{
		...
		$conditions = array('totalProductCount' => '',
							'activeProductCount' => ' WHERE Product.isEnabled = 1',
							'availableProductCount' => ' WHERE Product.isEnabled = 1 AND (stockCount > 0 OR type = ' . Product::TYPE_DOWNLOADABLE .  ')');
		...
		foreach ($conditions as $field => $condition)
		{
			...
			$queries[] = 'SELECT categoryID AS categoryID, COUNT(*) AS cnt, ' . $index . ' AS i FROM Product ' . $condition . ' GROUP BY categoryID';
		...
		$totalCounts = 'SELECT categoryID, SUM(cnt) AS cnt, i FROM (' . $basicCounts . ')  AS basicCounts WHERE categoryID IS NOT NULL GROUP BY categoryID, i';

		self::executeUpdate('CREATE TEMPORARY TABLE CategoryCount ' . $totalCounts);
		...
		foreach ($fields as $field)
		{
			$sql = 'UPDATE Category LEFT JOIN CategoryCount ON Category.ID=CategoryCount.categoryID AND CategoryCount.i=' . $field .' SET ';
			$sql .= 'Category.' . $field . '=CategoryCount.cnt';
			self::executeUpdate($sql);
		}
