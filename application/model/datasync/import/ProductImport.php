...
class ProductImport extends DataImport
{
	...
	public function importInstance($record, CsvImportProfile $profile)
	{
		...
		$cat = $this->getCategory($profile, $record, $extraCategories);
		...
		if (isset($fields['Product']) && $cat)
		{
			...
			if ($product)
			{
				...
				if (($cat instanceof Category) && (isset($fields['Category']) || isset($fields['Categories'])))
				{
	...
	private function getCategory(CsvImportProfile $profile, $record, &$extraCategories)
	{
		$fields = $this->getSortedFields($profile);
		...
		else if (isset($fields['Categories']['categoryIDs']))
		{
			$index = $fields['Categories']['categoryIDs'];

			preg_match_all('/([0-9]+)/', $record[$index], $res);
			$categories = array_shift($res);

			try
			{
				$cat = Category::getInstanceByID(array_shift($categories), true);
			}
			catch (Exception $e)
			{
			...
			$extraCategories = in(f('Category.ID'), $categories);
		}
	...
	private function importAdditionalCategories(CsvImportProfile $profile, Product $product, $extraCategories)
	{
		...
		else if ($extraCategories instanceof Condition)
		{
