...
class ProductVariationController extends StoreManagementController
{
	...
	public function save()
	{
		...
		{
			$f = new ARSelectFilter(new INCond(new ARFieldHandle('ProductVariation', 'typeID'), $existingTypes));
			$f->mergeCondition(new NotINCond(new ARFieldHandle('ProductVariation', 'ID'), $existingVariations));
			foreach (ActiveRecordModel::getRecordSet('ProductVariation', $f) as $variation)
			{
				foreach ($variation->getRelatedRecordSet('ProductVariationValue') as $val)
				{
					$val->delete();
				}
		...
		foreach ($items as $index => $id)
		{
			...
			{
				if ($item->$field->get() || isset($request[$field][$index]))
				{
