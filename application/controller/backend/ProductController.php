...
class ProductController extends ActiveGridController implements MassActionInterface
{
	...
	private $isDetailedExport = false;

	public function exportDetailed()
	{
		$this->isDetailedExport = true;
	...
	protected function getPreparedRecord($row, $displayedColumns)
	{
		...
		if (!empty($row['children']) && $this->isDetailedExport)
		{
...
//		if (isset($displayedColumns['Product.parentID']))
	...
	{
		$category = Category::getInstanceByID(1, Category::LOAD_DATA);
		$columns = array();
		$available = $this->getAvailableColumns($category, true);
		$displayed = $this->getDisplayedColumns($category);

		$displayed = array('Product.id', 'Product.name', 'Product.longDescription',  'Product.shortDescription',  'Product.URL', 'ProductImage.url', 'ProductPrice.price',  'Manufacturer.name');
		...
		foreach ($available as $column => $data)
		{
			if (substr($column, 0, 10) == 'specField.')
			{
				$columns[$column] = $data['name'];
			}
		}
...
*/
	...
	{
		$original = Product::getInstanceByID($this->request->get('id'), true);
		$cloned = clone $original;
		$cloned->isEnabled->set(false);
		$cloned->sku->set($cloned->sku->get() . ' (copy)');
		$cloned->save();

		return new JSONResponse($cloned->toFlatArray());
	}
