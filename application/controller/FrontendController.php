...
abstract class FrontendController extends BaseController
{
	...
	private function getTopCategories($rootID = 1)
	{
		ClassLoader::import('application.model.category.Category');

		if (!isset($this->topCategories[$rootID]))
		{
			$this->topCategories[$rootID] = Category::getInstanceByID($rootID, true)->getSubcategoryArray();
		}

		return $this->topCategories[$rootID];
	}
	...
	protected function boxCategoryBlock()
	{
		...
		// get top categories
		$topCategories = $this->getTopCategories($this->config->get('CAT_MENU_LEFT_ROOT'));
	...
	{
		$rootID = $this->config->get('CAT_MENU_LEFT_ROOT');
