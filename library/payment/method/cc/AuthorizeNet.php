...
class AuthorizeNet extends CreditCardPayment
{
	...
	private function initHandler()
	{
		...
		if ($this->getConfigValue('test') || (strtolower($this->getConfigValue('transactionKey')) == 'test'))
		{
		...
		if ($this->getConfigValue('gateway'))
		{
			...
			if (substr($this->gateway, 0, 4) != 'http')
			{
	...
	private function process()
	{
		...
		$res->currency->set($this->details->currency->get());
